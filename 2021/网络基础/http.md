# Http
#### 1.http1.0，http1.1 和 http2.0的区别
```js
...
#### 6.https加密原理
```js
[相关文档](https://www.jianshu.com/p/cf2f86bc597a)

Https的安全基础就是SSL，因此加密的详细内容就需要SSL。

SSL(安全套接字层)位于应用层和传输层之间，它可以为任何基于TCP等可靠连接的应用层协议提供安全性保证。
SSL协议本身分为两层：
...
2. 服务器回应
  ...
  - 支持的一些SSL/TLS扩展
3. 客户端回应
  - 一个随机数（Pre Master Secret）。该随机数用服务器公钥加密，防止被窃听
...
```js
// 持久化
HTTP 1.1 增加了 keep-alive 持久化的特性
持久连接使客户端到服务器端连接持续有效，避免了重新建立连接
大大减少了连接的建立以及关闭时延。
HTTP连接是建立在TCP协议之上的，建立一条TCP连接需要三次握手，TCP连接关闭时需要四次挥手

// 管线化
是指将多个 HTTP 请求整批发送，在发送过程中不用等待对方响应
管线化是在持久连接的基础上实现的
管线化的实现，能够同时并行发送多个请求，而不需要一个接一个的等待响应

// 持久连接和管线化的区别
持久连接的一个缺点是请求和响应式是顺序执行的，只有在请求1的响应收到之后，才会发送请求
而管线化不需要等待上一次请求得到响应就可以进行下一次请求。实现并行发送请求
...
```js
应用层
  - 定义了应用进程间的交互和通信规则，不同主机的应用进程间如何相互传递报文
  - 比如传递的报文的类型、格式、 有哪些字段等等
  - 例如HTTP协议和DNS协议
表示层
  - 数据的表示，安全，压缩
会话层
  - 建立，管理，终止会话
传输层
  - 为不同主机上的不同进程间提供了逻辑通信的功能。传输层只工作在端系统中
  - 例如 UDP 和 TCP 协议
网络层
  - 要实现了不同主机间的逻辑通信功能。
  - 网络层协议一共包含两个主要的组件，一个 IP 网际协议，一个是路由选 择协议
数据链路层
...
```
---
#### 13. TLS/SSL 中什么一定要用三个随机数，来生成"会话密钥"
```
...
```
---
#### 17. 当你在浏览器中输入 Google.com 并且按下回车之后发生了什么
```
...
```
---
#### 21. 网站域名加 www 与不加 www 的区别
```
在以前，www是告诉浏览器该访问是一个网站

删掉 www 的原因
  - 看起来很长
  - 不仅仅有www的资源，还有ftp等服务器资源

不要删掉 www 的原因
  - google在SEO的时候是区分www和不加www的
  - 删掉 www 后，老浏览器会有cookie问题
  - 删掉 www 后，上传图片等操作也会带上cookie，影响性能
```
---
